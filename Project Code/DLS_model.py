...
def prepare_features(asset_returns: np.ndarray, asset_prices: np.ndarray, lookback: int):
    ...
    for t in range(lookback, n_timesteps):
        past_prices = asset_prices[t - lookback:t]
        past_returns = asset_returns[t - lookback:t]
        features = np.concatenate([past_prices, past_returns], axis=1)
        X.append(features)
        Y.append(asset_returns[t])
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        if progress_report:
            if epoch % 10 == 0 or epoch == n_epochs - 1:
                print(f"Epoch {epoch}: Loss (−Sharpe) = {loss.item():.4f}")

    # disable gradinet tracking 
    with torch.no_grad(): # set requires grad to false, only for model evaluation / prediction. This is required for model training 
