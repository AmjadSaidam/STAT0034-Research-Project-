...
class PortfolioRNN(nn.Module):
    ...
    def forward(self, x, hidden=None):
        """
        Forward pass.
        dim(x) = (batch_size, sequence_length, input_size = n_assets * n_features)
        """
        if hidden is None:
        ...
        raw_weights = self.fc(out)  # (batch_size, seq_len=t, hidden_size=100) 
        
        if self.long_short:
            activated = self.L_activation(raw_weights) # transform ouput dimension by adding portfolio constraint linear transformation
...
def backprop_rnn(asset_returns, target_returns, target_volatility, hidden_dim=100, num_layers=2, 
                 ...
                 learning_rate=0.001, show_progress = False):
    """
    ...
    for epoch in tqdm(range(n_epochs)):
        weights = model(X_prior)  # (1, m-1, n_assets) # output = (batch_size, sequence_length, n_assets)
    ...
    with torch.no_grad(): # detach from computation graph 
        ...
        if long_short:
            final_weights = avg_weights / np.sum(np.abs(avg_weights) + 1e-8) # normalise to sum to 1
        else:
